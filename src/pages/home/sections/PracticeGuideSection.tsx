...
function buildPractice2Prompt(systemLine: string): string {
  return `같은 대화에서 이어서, 1단계에서 찾은 문제를 바탕으로 Lean Canvas를 만들어 줘.
목표는 나중에 제출 양식 5항목(제안 배경·AI 활용 시나리오·활용 데이터·관련 시스템·기대 효과)을 쓰기 쉽게 만드는 거야.
(나는 외부 라이브러리·AI 도구 이름은 잘 몰라. 그건 네가 제안해 줘.)

배경 정보
- 고른 문제(아이디어): [아이디어 내용 — 1단계에서 고른 문제]
- 적용 부서: [부서명]
...
2. 내가 고른 내부 시스템과 문제가 어떻게 이어지는지 **2~3문장**으로 적어 줘.
3. 외부 기술·라이브러리는 내가 몰라도 되게 쉬운 말로 적고, 이름은 네가 골라 줘.
4. 제출 문안 본문은 아직 작성하지 마.
...
const practices: Practice[] = [
  {
    ...
    steps: [
      ...
      '고른 문제와 "주 X시간 줄이고 싶다" 정도만 메모해 2단계로 갑니다',
    ],
...
답변은 한국어로 작성해 줘.`,
    followUps: [
      '추천해 준 문제 1개를 내 말로 다시 요약해 줘. 내가 2단계에 그대로 붙여넣을 수 있게.',
      '그 문제와 관련해 내가 시스템에서 자주 누르는 화면/버튼이 있을 법한 것을 추측해 물어봐 줘.',
    ],
  },
  {
    title: '2단계: 아이디어 Lean Canvas 작성',
    icon: '📋',
    duration: '25분',
    difficulty: '중급',
    steps: [
      '위에서 **우리 회사 내부 시스템**을 고릅니다 (여러 개 가능, 없으면 기타)',
      '1단계에서 고른 **문제 1개**를 노란 칸에 붙여넣습니다 (같은 ChatGPT 창)',
      '필요하면 「더 알려줄 말」만 짧게 적고 ChatGPT에 보냅니다',
      '완성된 Canvas·내부 시스템·외부 도구 제안은 3단계(제출 문안)의 재료로 씁니다',
    ],
    // 프롬프트는 buildPractice2Prompt()로 시스템 선택·더 알려줄 말 반영
    prompt: '',
    followUps: [
  ...
  {
    title: '3단계: 제출 양식용 최종 문안 만들기',
    icon: '📝',
    duration: '25분',
    difficulty: '중급',
    steps: [
      '2단계까지 대화 내용을 같은 창에 이어서 진행합니다',
...
반드시 반영할 것
- 1단계에서 찾은 **문제(페인포인트)**
- 2단계 Lean Canvas의 가치·기능·KPI·기대 효과
- 2단계에서 내가 고른 **내부 시스템** 명칭과 역할
- 네가 제안한 **외부 라이브러리·기술**(사용자가 모를 수 있으니 시나리오에 자연스럽게 녹여 줄 것)
- 「더 알려줄 말」과 내가 이어서 답한 내용
...
위 규칙을 지키며 5개 항목을 순서대로 출력해 줘.`,
  ...
  },
  {
    ...
    difficulty: '고급',
    steps: [
      '3단계에서 받은 5개 항목 문안 전체를 [제출 문안]에 붙여넣습니다',
                  ...
                  <div className="min-w-0">
                    ...
                    >
                      <span className="text-xs sm:text-sm shrink-0 text-muted-foreground font-extrabold tracking-wide">
                        {practice.title.match(/^\d+단계|^실습\s*\d+/)?.[0] ?? `실습 ${i + 1}`}
                      </span>
                      <span>
                        {practice.title.replace(/^(\d+단계|실습\s*\d+)\s*[:：]\s*/, '')}
                      </span>
                    </div>
                    ...
                    <div className="px-6 pb-6 border-t border-border pt-6 space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <EditablePrompt
                            text={i === 1 ? buildPractice2Prompt(systemLine) : practice.prompt}
